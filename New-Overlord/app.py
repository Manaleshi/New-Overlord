...
def get_hex_neighbors_debug(x, y, width, height, wrap_east_west=True):
    """
    Debug version - let's try the correct hex offset coordinate system
    """
    neighbors = {}
    
    # CORRECTED hexagonal offset coordinates
    # The issue was in my offset patterns - let me fix this
    
    if y % 2 == 0:  # Even rows (0, 2, 4, ...)
        offsets = {
            'N':  (0, -1),   # North: same column, row up
            'NE': (0, -1),   # Northeast: same column, row up (this was wrong!)
            'SE': (0, 1),    # Southeast: same column, row down  
            'S':  (0, 1),    # South: same column, row down
            'SW': (-1, 1),   # Southwest: column left, row down
            'NW': (-1, -1)   # Northwest: column left, row up
        }
    else:  # Odd rows (1, 3, 5, ...)
        offsets = {
            'N':  (0, -1),   # North: same column, row up
            'NE': (1, -1),   # Northeast: column right, row up
            'SE': (1, 1),    # Southeast: column right, row down
            'S':  (0, 1),    # South: same column, row down  
            'SW': (0, 1),    # Southwest: same column, row down (this was wrong!)
            'NW': (0, -1)    # Northwest: same column, row up
        }
    
    # Actually, let me use the STANDARD hex offset coordinate system:
    ...
    print(f"DEBUG: Calculating neighbors for hex ({x},{y}), row {'even' if y%2==0 else 'odd'}")
    
    for direction, (dx, dy) in offsets.items():
        new_x = x + dx
        new_y = y + dy
        
        print(f"  {direction}: ({x},{y}) + ({dx},{dy}) = ({new_x},{new_y})")
        
        # Handle north/south boundaries
        if new_y < 0 or new_y >= height:
            neighbors[direction] = None
            print(f"    -> BOUNDARY (y out of range)")
            continue
        ...
        if wrap_east_west:
            if new_x < 0:
                new_x = width - 1
                print(f"    -> WRAPPED to ({new_x},{new_y})")
            elif new_x >= width:
                new_x = 0
                print(f"    -> WRAPPED to ({new_x},{new_y})")
        else:
            if new_x < 0 or new_x >= width:
                neighbors[direction] = None
                print(f"    -> BOUNDARY (x out of range)")
                continue
        
        neighbors[direction] = (new_x, new_y)
        print(f"    -> VALID: ({new_x},{new_y})")
...
def test_hex_coordinates():
    ...
    print("=== TESTING HEX COORDINATES ===")
    
    # Test a few hexes in a 5x5 grid
    test_hexes = [(2, 2), (3, 2), (2, 3), (3, 3)]
    
    for x, y in test_hexes:
        print(f"\n--- Testing hex ({x},{y}) ---")
        neighbors = get_hex_neighbors_debug(x, y, 5, 5, True)
        for direction, coords in neighbors.items():
            if coords:
                print(f"{direction}: {coords}")
            else:
                print(f"{direction}: BOUNDARY")

# Add this route to test
@app.route('/debug-hex/<int:x>/<int:y>')
def debug_hex(x, y):
    """Debug hex coordinate calculation"""
    neighbors = get_hex_neighbors_debug(x, y, 5, 5, True)
    return jsonify({
        'hex': f'({x},{y})',
        'neighbors': neighbors,
        'row_type': 'even' if y % 2 == 0 else 'odd'
    })
